...
void EWBHand::setFingerSpeed(uint8_t fingerID, float speed)
{
    ...
    {
        if (startIdx + i < 14)
            _simulatedAngles[startIdx + i] += (speed * 5.0);
    }
#else
    Wire.beginTransmission(PCA9685_ADDR);
...
float EWBHand::readRawSensor(uint8_t sensorIndex)
{
    ...
    float rawValue;
#ifdef SIMULATION_MODE
    rawValue = _simulatedAngles[sensorIndex];
#else
    _selectMuxChannel(sensorIndex);
    rawValue = _readAS5600();
#endif

    // 1. Calculate raw difference
    float normalized = rawValue - _angleOffset[sensorIndex];

    //2. wraparound fix
    float circleMax = 360.0;

    //correct for underflow
    if (normalized < 0.0){
        normalized += circleMax;
    }
    //correct for overflow just in case
    else if (normalized >= circleMax){
        normalized -= circleMax;
    }
...
void EWBHand::calibrate()
{
    ...
    for (int i = 0; i < 14; i++)
        _angleOffset[i] = _simulatedAngles[i];
#else
    for (int i = 0; i < 14; i++)
    {
